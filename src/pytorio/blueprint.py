...
class Entity:
    def __init__(self,
        ...
        if items != None: self.items = items
    ...
    def __str__(self):
        ...
        if hasattr(self, "items"): e_str += ", " + str(self.items)
        e_str += ")"
    ...
    def to_dict(entity):
        ...
        if hasattr(entity, "request_filters"):
            entity_dict.update({
                "request_filters":
                [Logistic_Filter.to_dict(logistic_filter) for logistic_filter in entity.request_filters]
            })
        if hasattr(entity, "items"):
            entity_dict.update({"items": entity.items})
    ...
    def from_dict(entity_dict):
        ...
        return Entity(entity_dict["name"], Position.from_dict(entity_dict["position"]), direction, bar, recipe,
                      request_filters)
